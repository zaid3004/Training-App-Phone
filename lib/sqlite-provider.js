...
export function SQLiteProvider({ children }) {
  ...
  useEffect(() => {
    async function init() {
      ...
      // Phase 6: Add PRs table for per-user PR tracking
      await database.execAsync(`
      ...
      await database.execAsync(`
        ...
        CREATE INDEX IF NOT EXISTS idx_sets_log ON workout_sets(workout_log_id);
        CREATE INDEX IF NOT EXISTS idx_prs_user ON user_prs(user_id);
        CREATE INDEX IF NOT EXISTS idx_stats_user ON user_stats(user_id);
        CREATE INDEX IF NOT EXISTS idx_bodyweight_user_ts ON bodyweight_logs(user_id, ts DESC);
      `);
